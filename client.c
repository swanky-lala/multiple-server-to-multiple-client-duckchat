...
int main(int argc, char *argv[])
{
	...
	/* Client Program has 4 arguments
	 * 1-program name
	 * 2-addres of server
	 * 3-port to connect
	 * 4-username for user
	*/	
	if (argc != 4)
	{
		printf("Usage: ./client SERVER_ADDR SERVER_PORT USERNAME\n");
		exit(1);
	}
...
int send_login_message(char* username)
{
	...
	/*
	 * Check is whole message sent?
	 * do this: if( bytes != len) 
	*/
	if (bytes < 0)
	{
		return 1; //error
...
int send_leave_message(char* channel)
{
	...
	{
		// Are we going to leave active channel ?? 
		if (strcmp(channel,active_channel) == 0){

			//earse current value of active_channel
			memset(active_channel,0x00,sizeof(active_channel));

			//find new value from set

			map<string,string>::iterator it=subscribed_channels.begin();
			while(it != subscribed_channels.end()){
				/*
				 * Algorithm:
				 * First value from beginning that is NOT EQUAL to channel that we are deleting
				 * Will become new active_channel
				 *	
				*/
				std::string current_channel=it->second;
				if(strcmp(channel,current_channel.c_str())){
					strcpy(active_channel,current_channel.c_str());
					break;
				}
			}
		}
